...
class head extends Component {
    ...
    render() {
        ...
        return (
            ...
            <AddDetails onAddDetails={this.props.onAddName} />
           
            {/* {this.props.todosDetails.map(res =>{
                    <PersonDetails 
                    val={res.name} 
                    key={res.id}/>      
            })} */}
...
const mapStateToProps = state =>{ 
    return{
        todosDetails: state.todos 
    }
};
const mapDispatchToProps = (dispatch) => {
    return{
        onAddName: (value) => dispatch({type:'ADD_DETAILS', detail:{name:value}})
    }
